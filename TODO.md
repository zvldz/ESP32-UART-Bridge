...
- [ ] **Navigation via mDNS hostname (`.local`)** 🟡 MEDIUM PRIORITY
  ...
  Important: `.local` resolves via mDNS (multicast UDP:5353), NOT regular DNS (UDP:53).
  In AP mode captive DNS (UDP:53) never sees `.local` queries — only mDNS responder works.

  **Scenarios (Save & Reboot):**
  - AP→AP (no mode change): IP=192.168.4.1 unchanged, mDNS also works, fast reconnect
  - Client→Client (no mode change): DHCP IP may change, mDNS finds new IP
  - AP→Client (mode change): 192.168.4.1 ceases to exist, Windows ~15-20 sec to switch WiFi, mDNS only
  - Client→AP (mode change): old DHCP IP gone, mDNS finds 192.168.4.1
  - **Hostname change**: after Save JS knows new hostname from form → reconnect to `new-hostname.local`

  **Stage 1: Captive Portal redirect**
  - Current: handleNotFound() does `redirect("/")`, URL stays msftconnecttest.com
  - Solution: redirect to `http://{hostname}.local/` (always mDNS, no IP fallback)
  - Captive portal = always AP mode, mDNS responder on ESP active
  - Hostname always current: redirect takes hostname from ESP config
  - File: web_interface.cpp, handleNotFound()

  **Stage 2: Save & Reboot smart reconnect**
  - Current: Save → 10 sec countdown → `window.location.reload()` — single attempt, no retry
  - Problem: on mode change IP changes, Windows needs ~15-20 sec to switch WiFi
  - Solution: switch Save to `startReconnect()` from utils.js (already exists for firmware update)
    - `startReconnect()`: waitSeconds=8, then up to 30 retries every second
    - Each attempt: `Promise.any([fetch(mdnsUrl), fetch(currentUrl)])` — race
    - mdnsUrl = `http://{hostname from form}.local/` (handles hostname change)
    - currentUrl = `window.location.origin` (speeds up reconnect if IP unchanged)
    - If all 30 attempts fail — show link to `{hostname}.local`
  - Files: index.html (Save button), utils.js (startReconnect)
