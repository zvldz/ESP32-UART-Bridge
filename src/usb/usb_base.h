...
public:
    UsbBase(uint32_t baud) : baudrate(baud), initialized(false), rx_head(0), rx_tail(0), tx_head(0), tx_tail(0), task_handle(nullptr), is_connected(false) {
    ...
    int available() override {
        ...
        size_t count = (rx_head >= rx_tail) ? (rx_head - rx_tail) : (RX_BUFFER_SIZE - rx_tail + rx_head);
    ...
    int availableForWrite() override {
        ...
        size_t space = (tx_tail > tx_head) ? (tx_tail - tx_head - 1) : (TX_BUFFER_SIZE - tx_head + tx_tail - 1);
